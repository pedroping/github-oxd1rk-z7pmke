...
export class SingleSelectionExampleComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    this.form = new FormGroup({
      bankCtrl: new FormControl<Bank>(null),
      bankFilterCtrl: new FormControl<string>('')
    })
    // set initial selection
    this.form.get('bankCtrl')?.setValue(this.banks[10]);

    // load the initial bank list
    this.filteredBanks.next(this.banks.slice());

    // listen for search field value changes
    this.form.get('bankFilterCtrl')?.valueChanges
